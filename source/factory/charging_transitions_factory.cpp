...
charging_transitions_factory::charging_transitions_factory(const EV_EVSE_inventory& inventory, const EV_ramping_map& custom_EV_ramping, const EV_EVSE_ramping_map& custom_EV_EVSE_ramping)
    : inventory(inventory), custom_EV_ramping(custom_EV_ramping), custom_EV_EVSE_ramping(custom_EV_EVSE_ramping), charging_transitions_obj(this->load_charging_transitions()) {}
...
const charging_transitions charging_transitions_factory::load_charging_transitions()
{
    ...
    transitions_by_EVSE_level.emplace(L1, [&]() {
        ...
        return std::move(integrate_X_through_time(target_deadband, off_deadband, pos_and_neg_transitions_are_unique,
...
const integrate_X_through_time& charging_transitions_factory::get_charging_transitions(const EV_type& EV, const EVSE_type& EVSE) const
{
