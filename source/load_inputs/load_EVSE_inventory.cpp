...
EVSE_inventory load_EVSE_inventory::load(const std::string& inputs_dir)
{
    ...
    ASSERT( \
        elements_in_line.size() == column_names.size(), \
        EVSE_inputs_file << " invalid number of columns. Need " << column_names.size() << " columns but " << elements_in_line.size() << " provided in line number " << line_number \
    );

    // Check we have the correct columns
    {
        std::stringstream assert_error_ss;
        assert_error_ss << EVSE_inputs_file << " column names don\'t match. " << "Expected column names are { ";
        for( int i = 0; i < column_names.size(); i++ )
        {
            assert_error_ss << column_names.at(i);
        ...
        assert_error_ss << " }";
        ASSERT( elements_in_line == column_names, assert_error_ss.str() );
    }
