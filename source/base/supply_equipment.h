...
public:
    ...
    void get_CE_FICE( const FICE_inputs inputs,
                      bool& pev_is_connected_to_SE,
                      CE_FICE& return_val );

    void get_active_CE( bool& pev_is_connected_to_SE,
    ...
    void get_time_to_complete_active_charge_hrs( const double setpoint_P3kW,
                                                 bool& pev_is_connected_to_SE,
                                                 double& time_to_complete_charge_hrs );
    
    void get_active_charge_profile_forecast_allInfo( const double setpoint_P3kW,
                                                     const double time_step_mins,
                                                     bool& pev_is_connected_to_SE,
                                                     std::vector<pev_charge_profile_result>& charge_profile );
    
    void get_active_charge_profile_forecast_akW( const double setpoint_P3kW,
                                                 const double time_step_mins,
                                                 bool& pev_is_connected_to_SE,
    ...
    bool current_CE_is_using_control_strategy( const double unix_time_of_interest,
                                               const L2_control_strategies_enum control_enum_ );
    
    void get_external_control_strategy( std::string& return_val );
    
    bool current_CE_is_using_external_control_strategy( const std::string& ext_control );
