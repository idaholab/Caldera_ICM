...
void supply_equipment::set_pointers_in_SE_Load(factory_EV_charge_model* PEV_charge_factory, factory_ac_to_dc_converter* ac_to_dc_converter_factory, pev_charge_profile_library* charge_profile_library)
{
    this->SE_Load.set_pointers(PEV_charge_factory, ac_to_dc_converter_factory, charge_profile_library);
}
   

bool supply_equipment::is_SE_with_id(SE_id_type SE_id)
{
...
void supply_equipment::set_ensure_pev_charge_needs_met_for_ext_control_strategy(bool ensure_pev_charge_needs_met)
{
    this->SE_control.set_ensure_pev_charge_needs_met_for_ext_control_strategy(ensure_pev_charge_needs_met);
}


void supply_equipment::get_CE_FICE(FICE_inputs inputs, bool& pev_is_connected_to_SE, CE_FICE& return_val)
{
...
void supply_equipment::get_active_CE(bool& pev_is_connected_to_SE, active_CE& active_CE_val)
{
...
void supply_equipment::get_time_to_complete_active_charge_hrs(double setpoint_P3kW, bool& pev_is_connected_to_SE, double& time_to_complete_charge_hrs)
{
    this->SE_Load.get_time_to_complete_active_charge_hrs(setpoint_P3kW, pev_is_connected_to_SE, time_to_complete_charge_hrs);
}


void supply_equipment::get_active_charge_profile_forecast_allInfo(double setpoint_P3kW, double time_step_mins, bool& pev_is_connected_to_SE, std::vector<pev_charge_profile_result>& charge_profile)
{
...
void supply_equipment::get_active_charge_profile_forecast_akW(double setpoint_P3kW, double time_step_mins, bool& pev_is_connected_to_SE, std::vector<double>& charge_profile)
{
...
void supply_equipment::set_target_acP3_kW(double target_acP3_kW)
{
    this->SE_Load.set_target_acP3_kW(target_acP3_kW);
}


void supply_equipment::set_target_acQ3_kVAR(double target_acQ3_kVAR)
{
    this->SE_Load.set_target_acQ3_kVAR(target_acQ3_kVAR);
}


bool supply_equipment::pev_is_connected_to_SE(double now_unix_time)
{
    return this->SE_Load.pev_is_connected_to_SE(now_unix_time);
}


void supply_equipment::get_next(double prev_unix_time, double now_unix_time, double pu_Vrms, double& soc, ac_power_metrics& ac_power)
{
...
bool supply_equipment::current_CE_is_using_external_control_strategy(std::string& ext_control)
{
...
bool supply_equipment::current_CE_is_using_control_strategy(double unix_time_of_interest, L2_control_strategies_enum control_enum_)
{
...
void supply_equipment::ES500_get_charging_needs(double unix_time_now, double unix_time_begining_of_next_agg_step, ES500_aggregator_pev_charge_needs& pev_charge_needs)
{
    this->SE_control.ES500_get_charging_needs(unix_time_now, unix_time_begining_of_next_agg_step, this->SE_Load, pev_charge_needs);
}


void supply_equipment::ES500_set_energy_setpoints(double e3_setpoint_kWh)
{
